...
class SoundManager {
    constructor() {
        this.enabled = localStorage.getItem('socialStorySound') !== 'off';
        this.ctx = null;
    }
    ...
    toggle() {
        ...
        localStorage.setItem('socialStorySound', this.enabled ? 'on' : 'off');
        return this.enabled;
    }
}
...
class StoryGame {
    constructor() {
        ...
        this.storySteps = [
            {
                ...
                question: 'למי הפוסט/רילס הזה מיועד? תארו את העוקב האידיאלי שלכם.',
        ...
        this.mentorMessages = {
            welcome: [
                "היי! אני נועה, ואני כאן ללוות אותך בדרך ליצור תוכן שעוצר את הסקרול!",
                "ברוכים הבאים! אני נועה, המנטור שלך לתוכן שמושך עוקבים ולקוחות.",
                "שמחה לראות אותך! יחד נלמד ליצור תוכן שמשנה עסקים."
            ],
            moduleIntro: {
                1: "ההוק הוא המלך! בואו נלמד איך לכתוב שורה ראשונה שעוצרת את האצבע על המסך.",
                2: "מבנה טוב = פוסט שקוראים עד הסוף. בואו נלמד את הנוסחאות שעובדות!",
                3: "כל פלטפורמה = שפה אחרת. בואו נלמד לדבר באינסטגרם, לינקדאין וטיקטוק.",
                4: "רגש ואותנטיות הם מה שמבדיל תוכן בינוני ממצוין. מוכנים לרמה הבאה?",
                5: "עכשיו נלמד להפוך עוקבים ללקוחות - בלי להרגיש 'מוכרנים'. זה הקסם!"
            },
            lessonStart: [
                "מעולה! בואו נתחיל. קחו את הזמן, אין לחץ.",
                "אני לצידך בכל שלב. בהצלחה!",
                "זוכרים - כל תשובה נכונה מקרבת אתכם להיות יוצרי תוכן מדהימים!"
            ],
            correctAnswer: [
                "מדהים! בדיוק ככה!",
            ...
            lessonComplete: [
                ...
                "מעולה! עוד צעד לקראת תוכן שעוצר את הסקרול!",
        ...
        if (user) {
            try {
                ...
                if (data && !error) {
                    ...
                    localStorage.setItem('socialStoryData', JSON.stringify(this.playerData));
                    return;
                }
    ...
    loadFromLocalStorage() {
        try {
            const saved = localStorage.getItem('socialStoryData');
    ...
    async createSupabaseRow(user) {
        if (!user) return;
        try {
            await _supabase.from('social_story_players').upsert({
                user_id: user.id,
    ...
    async saveToSupabase() {
        ...
        try {
            await _supabase.from('social_story_players').update({
                xp: this.playerData.xp,
                level: this.playerData.level,
                hearts: this.playerData.hearts,
                max_hearts: this.playerData.maxHearts,
    ...
    renderOnboardingStep() {
        const steps = [
            {
                icon: '🎯',
                title: 'ברוכים הבאים לסטורי סושיאל!',
                text: 'כאן תלמדו ליצור תוכן שעוצר את הסקרול - צעד אחרי צעד, בדרך כיפית ואינטראקטיבית.'
            },
            {
                icon: '👩‍🏫',
                title: 'הכירו את נועה, המנטורית שלכם',
                text: 'נועה תלווה אתכם בכל שלב - תיתן טיפים, עידוד, ותעזור לכם להבין מה הופך סיפור לתוכן שעוצר את הסקרול.'
            },
            {
                icon: '🚀',
                title: 'בואו נתחיל!',
        ...
        return `
            <div class="mentor-container mentor-enter">
                <div class="mentor-avatar mentor-${mood}">
                    <img src="mentor-noa.png" alt="נועה" />
                </div>
                <div class="mentor-bubble">
                    ${showName ? '<div class="mentor-name">נועה - המנטורית שלך</div>' : ''}
        ...
        pathNodesHtml += `
            <div class="home-path-node special" onclick="game.transitionTo(function() { game.startStoryBuilder() })">
                ...
                <div class="home-path-info">
                    ...
                    <div class="home-path-desc">צרו סיפור שלם עם ליווי של נועה</div>
                </div>
        ...
        container.innerHTML = `
            ...
            <div class="exercise-container">
                ...
                <div class="mentor-tip">
                    <img class="mentor-tip-icon" src="mentor-noa.png" alt="נועה" />
