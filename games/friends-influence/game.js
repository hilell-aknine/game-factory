...
class SoundManager {
    constructor() {
        this.enabled = localStorage.getItem('friendsInfluenceSound') !== 'off';
        this.ctx = null;
    }
    ...
    toggle() {
        ...
        localStorage.setItem('friendsInfluenceSound', this.enabled ? 'on' : 'off');
        return this.enabled;
    }
}
...
class StoryGame {
        ...
        if (user) {
            try {
                const { data, error } = await _supabase
                    .from('friends_influence_players')
                    .select('*')
                    .eq('user_id', user.id)
                    .single();
                ...
                if (data && !error) {
                    ...
                    localStorage.setItem('friendsInfluenceData', JSON.stringify(this.playerData));
                    return;
                }
    ...
    loadFromLocalStorage() {
        try {
            const saved = localStorage.getItem('friendsInfluenceData');
    ...
    async createSupabaseRow(user) {
        if (!user) return;
        try {
            await _supabase.from('friends_influence_players').upsert({
                user_id: user.id,
    ...
    async saveToSupabase() {
        ...
        try {
            await _supabase.from('friends_influence_players').update({
                xp: this.playerData.xp,
                level: this.playerData.level,
                hearts: this.playerData.hearts,
                max_hearts: this.playerData.maxHearts,
