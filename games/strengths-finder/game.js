...
class SoundManager {
    constructor() {
        this.enabled = localStorage.getItem('strengthsFinderSound') !== 'off';
        this.ctx = null;
    }
    ...
    toggle() {
        ...
        localStorage.setItem('strengthsFinderSound', this.enabled ? 'on' : 'off');
        return this.enabled;
    }
}
...
class StrengthsFinderGame {
    constructor() {
        ...
        this.mentorMessages = {
            welcome: [
                "היי! אני עוז, ואני כאן ללוות אותך בדרך לגלות את החוזקות הייחודיות שלך!",
                "ברוכים הבאים! אני עוז, המנטור שלך לגילוי כישרונות וחוזקות.",
                "שמח לראות אותך! יחד נגלה את 34 נושאי החוזקה של גאלופ ונלמד למקסם את הפוטנציאל שלך."
            ],
            moduleIntro: {
                1: "הצעד הראשון הוא להכיר את עצמנו! בואו נגלה מהם נושאי החוזקה ואיך הם עובדים.",
                2: "חוזקות ביצוע הן המנוע שלכם! בואו נלמד על Achiever, Discipline, Focus ועוד.",
                3: "חוזקות השפעה עוזרות לכם להוביל אחרים! נגלה את Command, Communication, Woo ועוד.",
                4: "חוזקות בניית קשרים הן הדבק שמחבר צוותים! נכיר את Empathy, Harmony, Relator ועוד.",
                5: "חוזקות חשיבה אסטרטגית עוזרות לכם לקבל החלטות חכמות! נלמד על Strategic, Ideation, Analytical ועוד."
            },
            lessonStart: [
                "מעולה! בואו נתחיל לגלות עוד חוזקה. קחו את הזמן, אין לחץ.",
                "אני לצידך בכל שלב של המסע. בהצלחה!",
                "זוכרים - כל תשובה נכונה מקרבת אתכם להכיר טוב יותר את הכישרונות שלכם!"
            ],
            correctAnswer: [
                "מדהים! בדיוק ככה!",
                "וואו, את/ה ממש מבין/ה את החוזקות!",
                "נכון מאוד! יש לך חוש מצוין לזהות כישרונות!",
                "יופי! ככה בדיוק ממקסמים חוזקות!",
                "מצוין! את/ה בדרך להפוך למאסטר החוזקות!"
            ],
            wrongAnswer: [
                "לא נורא! טעויות הן חלק מהלמידה והגילוי.",
                "קרוב! בפעם הבאה חשבו איזו חוזקה באמת מתארת את ההתנהגות הזו.",
                "לא בדיוק, אבל זו הזדמנות ללמוד משהו חדש על עצמכם!",
                "אל דאגה, גם המומחים הגדולים טעו בהתחלה!"
            ],
            encouragement: [
                "את/ה עושה עבודה נהדרת בגילוי החוזקות!",
                "כל תרגיל מקרב אותך להכרת הכישרונות שלך!",
                "אני גאה בך! ממשיכים לגלות!",
                "המשיכו ככה - כל חוזקה שתגלו תשנה לכם את החיים!"
            ],
            lessonComplete: [
                "איזה כיף! גילית עוד חוזקות חדשות!",
                "מעולה! עוד צעד לקראת הכרת כל 34 נושאי החוזקה!",
                "אלופים! הידע הזה יעזור לכם למקסם את הפוטנציאל שלכם!"
            ]
        };
        ...
        this.achievements = [
            ...
            { id: 'five_lessons', title: 'חוקר כישרונות', desc: 'סיימת 5 שיעורים', icon: '📚', condition: (data) => Object.keys(data.completedLessons).length >= 5 },
            { id: 'all_lessons', title: 'מאסטר החוזקות', desc: 'סיימת את כל השיעורים', icon: '👑', condition: (data) => Object.keys(data.completedLessons).length >= 15 },
            { id: 'streak_3', title: 'התמדה', desc: '3 ימים ברצף', icon: '🔥', condition: (data) => data.streak >= 3 },
            { id: 'streak_7', title: 'שבוע של צמיחה', desc: '7 ימים ברצף', icon: '⭐', condition: (data) => data.streak >= 7 },
            ...
            { id: 'storyteller', title: 'מספר סיפורים', desc: 'כתבתם את הסיפור הראשון', icon: '📖', condition: (data) => (data.storiesCreated || 0) >= 1 },
            { id: 'story_master', title: 'סופר מיומן', desc: 'כתבתם 3 סיפורים', icon: '✍️', condition: (data) => (data.storiesCreated || 0) >= 3 }
        ];
    ...
    initRipple() {
        document.addEventListener('click', (e) => {
            const target = e.target.closest('.btn, .option-btn, .word-chip, .compare-card, .module-card, .lesson-item, .daily-challenge-btn, .practice-mode-btn, .review-btn, .path-node:not(.locked), .home-path-node:not(.locked)');
            if (!target) return;
        ...
        if (user) {
            try {
                ...
                if (data && !error) {
                    ...
                    localStorage.setItem('strengthsFinderData', JSON.stringify(this.playerData));
                    return;
                }
    ...
    loadFromLocalStorage() {
        try {
            const saved = localStorage.getItem('strengthsFinderData');
    ...
    async createSupabaseRow(user) {
        if (!user) return;
        try {
            await _supabase.from('strengths_finder_players').upsert({
                user_id: user.id,
                ...
                review_queue: this.playerData.reviewQueue || [],
                last_review_date: this.playerData.lastReviewDate
            }, { onConflict: 'user_id' });
        } catch (e) {
    ...
    async saveToSupabase() {
        ...
        try {
            await _supabase.from('strengths_finder_players').update({
                xp: this.playerData.xp,
                level: this.playerData.level,
                hearts: this.playerData.hearts,
                max_hearts: this.playerData.maxHearts,
    ...
    renderOnboardingStep() {
        const steps = [
            {
                icon: '💪',
                title: 'ברוכים הבאים למאסטר החוזקות!',
                text: 'כאן תגלו את 34 נושאי החוזקה של גאלופ ותלמדו למקסם את הכישרונות שלכם.'
            },
            {
                icon: '🎯',
                title: 'איך זה עובד?',
                text: 'תרגילים אינטראקטיביים, XP, סטריקים ולבבות - כל תשובה נכונה מקרבת אתכם להכיר את החוזקות שלכם.'
            },
            {
                icon: '🏆',
                title: '5 תחומי חוזקה',
                text: 'מבוא, ביצוע, השפעה, בניית קשרים וחשיבה אסטרטגית - 34 כישרונות שישנו לכם את החיים.'
            }
        ];
        ...
        return `
            <div class="mentor-container mentor-enter">
                ...
                <div class="mentor-bubble">
                    ${showName ? '<div class="mentor-name">עוז - המנטור שלך</div>' : ''}
    ...
    getLevelName(level) {
        if (level >= 10) return 'מאסטר החוזקות';
        if (level >= 8) return 'מאיץ כוחות';
        if (level >= 6) return 'מפתח פוטנציאל';
        if (level >= 4) return 'מגלה חוזקות';
        if (level >= 2) return 'חוקר כישרונות';
        return 'מתחיל סקרן';
    }
    ...
    renderReviewButton() {
        const reviewCount = (this.playerData.reviewQueue || []).length;
        if (reviewCount === 0) return '';
        return `
            <button class="review-btn" onclick="game.transitionTo(function() { game.startReviewMode() })">
                <div class="review-btn-icon">🔄</div>
                <div class="review-btn-info">
                    <div class="review-btn-title">חזרה ותרגול</div>
                    <div class="review-btn-desc">חזרו על תרגילים שטעיתם בהם</div>
                </div>
                <div class="review-badge">${reviewCount}</div>
            </button>
        `;
    }
    ...
    renderHomeScreen() {
        ...
        pathNodesHtml += `
            <div class="home-path-node goal" style="cursor: default;">
                ...
                <div class="home-path-info">
                    ...
                    <div class="home-path-title">מאסטר החוזקות</div>
                    <div class="home-path-desc">השלימו את כל המודולים וגלו את כל 34 הכישרונות</div>
                    <div class="home-path-progress">
                        <div class="home-path-progress-bar">
        ...
        container.innerHTML = `
            ...
            <!-- Story Builder + Review Mode -->
            <div style="margin-bottom: 20px;">
                ${this.renderReviewButton()}
                <button class="practice-mode-btn" onclick="game.transitionTo(function() { game.renderStoryBuilder() })">
                    <div class="practice-mode-icon">📖</div>
                    <div class="practice-mode-info">
                        <div class="practice-mode-title">בונה הסיפורים</div>
                        <div class="practice-mode-desc">צרו סיפור אישי על החוזקות שלכם${this.playerData.storiesCreated > 0 ? ` (${this.playerData.storiesCreated} נכתבו)` : ''}</div>
                    </div>
                </button>
            </div>
    ...
    createExerciseTip(exerciseType) {
        const tips = {
            'multiple-choice': "💡 חשבו איזו חוזקה באמת מתארת את ההתנהגות - זו בדרך כלל התשובה הנכונה!",
            'fill-blank': "💡 המילה הנכונה היא זו שמתחברת הכי טוב לעולם החוזקות של גאלופ.",
            'order': "💡 חשבו על הסדר ההגיוני - מהבסיס לפסגה.",
            'identify': "💡 חפשו את החלק שמתאר התנהגות אופיינית של חוזקה מסוימת.",
            'compare': "💡 דמיינו מצב אמיתי - איזו חוזקה באמת מתבטאת כאן?",
            'improve': "💡 חפשו את האפשרות שמדייקת את ההבנה של החוזקה.",
    ...
    renderImprove(container, exercise) {
        ...
        container.innerHTML = `
            <button class="back-btn" onclick="game.exitLesson()">✕</button>
            <div class="exercise-container">
                <div class="exercise-type">שפר את ההגדרה</div>
                <div class="exercise-question">${exercise.question}</div>
    ...
    checkAnswer() {
        ...
        if (isCorrect) {
            ...
            // Review mode: remove from queue if answered correctly
            if (this.isReviewMode && exercise._reviewKey) {
                this.removeFromReviewQueue(exercise._reviewKey);
                this.reviewCorrectCount = (this.reviewCorrectCount || 0) + 1;
            }
        } else {
            ...
            // Add to review queue for future practice (skip if already in review mode)
            if (!this.isReviewMode && this.currentModule && this.currentLesson) {
                this.addToReviewQueue(this.currentModule.id, this.currentLesson.id, this.currentExerciseIndex);
            }
        }

        // Track stats
        const statModuleId = exercise._moduleId || (this.currentModule && this.currentModule.id) || 1;
        this.updateExerciseStats(isCorrect, statModuleId);
    ...
    // ═══════════════════════════════════════
    completeLesson() {
        // Review mode uses its own completion
    ...
    storySteps = [
        {
            title: "שלב 1 - זיהוי",
            question: "מהי החוזקה שאתם הכי מזדהים איתה?",
            placeholder: "למשל: אמפתיה, אסטרטגיה, אחריות, לימוד...",
            example: "אני מזדהה מאוד עם חוזקת ה'לימוד' - תמיד אני מרגיש צורך ללמוד דברים חדשים ולהתפתח."
        },
        {
            title: "שלב 2 - סיטואציה",
            question: "תארו מצב אמיתי שבו החוזקה הזו באה לידי ביטוי",
            placeholder: "ספרו על רגע ספציפי מהחיים שלכם...",
            example: "בעבודה הקודמת שלי, כשהוצג מוצר חדש - הייתי הראשון שלמד אותו לעומק ועזרתי לכל הצוות להבין את היתרונות שלו."
        },
        {
            title: "שלב 3 - השפעה יומיומית",
            question: "איך החוזקה הזו משפיעה על החיים שלכם ביום-יום?",
            placeholder: "חשבו על הרגלים, בחירות, ואינטראקציות יומיות...",
            example: "כל יום אני מקדיש לפחות שעה ללמידת משהו חדש - בין אם זה פודקאסט, ספר או קורס אונליין. זה נותן לי אנרגיה."
        },
        {
            title: "שלב 4 - התוצאה",
            question: "מה קורה כשאתם חיים את החוזקה הזו במלואה?",
            placeholder: "מהי ההרגשה? מה התוצאות?",
            example: "כשאני לומד ומתפתח, אני מרגיש חי ומלא מוטיבציה. אנשים סביבי פונים אליי לעצות כי הם יודעים שאני תמיד מעודכן."
        }
    ];

    renderStoryBuilder() {
        this.currentScreen = 'story';
        this.storyCurrentStep = 0;
        if (!this.storyAnswers) this.storyAnswers = ['', '', '', ''];

        const container = document.getElementById('game-container');
        this._renderStoryStep(container);
        this.hideProgressBar();
        this.hideFooter();
    }

    _renderStoryStep(container) {
        const step = this.storySteps[this.storyCurrentStep];
        const total = this.storySteps.length;

        const dotsHtml = this.storySteps.map((_, i) => {
            let cls = 'story-step-dot';
            if (i === this.storyCurrentStep) cls += ' active';
            else if (this.storyAnswers[i] && this.storyAnswers[i].trim()) cls += ' completed';
            return `<div class="${cls}"></div>`;
        }).join('');

        const isLast = this.storyCurrentStep === total - 1;
        const isFirst = this.storyCurrentStep === 0;
        const currentAnswer = this.storyAnswers[this.storyCurrentStep] || '';
        const allFilled = this.storyAnswers.every(a => a && a.trim().length > 0);

        container.innerHTML = `
            <div class="story-builder">
                <button class="back-btn" onclick="game.exitStoryBuilder()">→ חזרה</button>

                <div class="story-steps">${dotsHtml}</div>

                <div class="story-step-content">
                    <div class="story-step-title">${step.title}</div>
                    <div class="story-step-question">${step.question}</div>
                    <textarea class="story-textarea" id="story-textarea"
                        placeholder="${step.placeholder}"
                        oninput="game.storyUpdateAnswer(${this.storyCurrentStep}, this.value)">${currentAnswer}</textarea>
            ...
            <div class="story-nav">
                <div class="container">
                    ${!isFirst ? `<button class="btn btn-secondary" onclick="game.storyGoToStep(${this.storyCurrentStep - 1})">הקודם</button>` : '<div></div>'}
                    ${isLast
                        ? `<button class="btn btn-primary" ${!allFilled ? 'disabled' : ''} onclick="game.storyFinish()">סיום וצפייה</button>`
                        : `<button class="btn btn-primary" onclick="game.storyGoToStep(${this.storyCurrentStep + 1})">הבא</button>`
                    }
                </div>
            </div>
        `;
    ...
    storyUpdateAnswer(stepIndex, value) {
        if (!this.storyAnswers) this.storyAnswers = ['', '', '', ''];
        this.storyAnswers[stepIndex] = value;

        // Update finish button state
        const allFilled = this.storyAnswers.every(a => a && a.trim().length > 0);
        const finishBtn = document.querySelector('.story-nav .btn-primary');
        if (finishBtn && finishBtn.textContent.includes('סיום')) {
    ...
    storyFinish() {
        ...
        const container = document.getElementById('game-container');
        const sectionsHtml = this.storySteps.map((step, i) => `
            <div class="story-preview-section">
                <div class="story-preview-label">${step.title}</div>
                <div class="story-preview-text">${this.storyAnswers[i]}</div>
            </div>
        `).join('');
        ...
        container.innerHTML = `
            <div class="story-builder">
                <div class="completion-icon">📖</div>
                <div class="completion-title">הסיפור שלכם מוכן!</div>
                <div class="completion-subtitle">+30 XP</div>

                <div class="story-preview">
                    <div class="story-preview-title">✨ סיפור החוזקה שלי ✨</div>
                    ${sectionsHtml}
                </div>

                <div class="story-complete-actions">
                    <button class="btn btn-primary btn-full copy-btn" onclick="game.storyCopyToClipboard()">
                        📋 העתקה ללוח
                    </button>
                    <button class="btn btn-secondary btn-full" onclick="game.storyAnswers = ['','','','']; game.renderStoryBuilder()">
                        ✍️ כתיבת סיפור חדש
                    </button>
        ...
        if (storyNav) storyNav.remove();
    }

    storyCopyToClipboard() {
        const storyText = this.storySteps.map((step, i) =>
            `${step.title}\n${step.question}\n${this.storyAnswers[i]}`
        ).join('\n\n');

        const fullText = `✨ סיפור החוזקה שלי ✨\n\n${storyText}`;

        navigator.clipboard.writeText(fullText).then(() => {
            const btn = document.querySelector('.copy-btn');
            if (btn) {
                btn.textContent = '✅ הועתק!';
                setTimeout(() => { btn.textContent = '📋 העתקה ללוח'; }, 2000);
            }
        }).catch(() => {
            // Fallback
    ...
    // ═══════════════════════════════════════
    addToReviewQueue(moduleId, lessonId, exerciseIndex) {
        if (!this.playerData.reviewQueue) this.playerData.reviewQueue = [];
        const key = `${moduleId}-${lessonId}-${exerciseIndex}`;
        if (!this.playerData.reviewQueue.includes(key)) {
            this.playerData.reviewQueue.push(key);
        }
    }

    removeFromReviewQueue(key) {
        if (!this.playerData.reviewQueue) return;
        this.playerData.reviewQueue = this.playerData.reviewQueue.filter(k => k !== key);
    }

    getReviewExercises(count = 6) {
        const queue = this.playerData.reviewQueue || [];
        if (queue.length === 0) return [];

        // Pick random exercises from the queue
        const shuffled = this.shuffleArray([...queue]);
        const selected = shuffled.slice(0, count);

        const exercises = [];
        for (const key of selected) {
            const [modId, lesId, exIdx] = key.split('-').map(Number);
            ...
            exercises.push({ ...exercise, _reviewKey: key, _moduleId: modId });
        }
        return exercises;
    }

    startReviewMode() {
        const exercises = this.getReviewExercises(6);
        if (exercises.length === 0) return;

        this.sound.play('click');
        this.currentScreen = 'exercise';
        this.isReviewMode = true;
        this.reviewExercises = exercises;
        this.currentExerciseIndex = 0;
        this.lessonMistakes = 0;
        this.reviewCorrectCount = 0;

        // Create a virtual lesson for the review
        this.currentLesson = {
            title: 'חזרה ותרגול',
            exercises: exercises
        };
        // Track module for stats
        this.currentModule = this.currentModule || MODULES[0];
    ...
    completeReview() {
        ...
        const totalReviewed = this.reviewExercises.length;
        const correctCount = this.reviewCorrectCount || 0;
        const bonusXP = correctCount * 5;
        if (bonusXP > 0) this.addXP(bonusXP);

        this.playerData.lastReviewDate = new Date().toDateString();
        this.savePlayerData();
        ...
        container.innerHTML = `
            <div class="completion-screen">
                <div class="completion-icon">🔄</div>
                <div class="completion-title">חזרה הושלמה!</div>
                <div class="completion-subtitle">עברתם על ${totalReviewed} תרגילים</div>
                <div class="completion-stats">
                    <div class="completion-stat">
                        <div class="completion-stat-value">${correctCount}/${totalReviewed}</div>
                        <div class="completion-stat-label">תשובות נכונות</div>
                    </div>
                    <div class="completion-stat">
                        <div class="completion-stat-value">+${bonusXP}</div>
                        <div class="completion-stat-label">XP בונוס</div>
                    </div>
                </div>
                <div style="text-align: center; margin: 15px 0; color: var(--text-secondary); font-size: 14px;">
                    ${(this.playerData.reviewQueue || []).length > 0
                        ? `נשארו עוד ${this.playerData.reviewQueue.length} תרגילים לחזרה`
                        : '🎉 אין תרגילים לחזרה - מצוין!'}
                </div>
                <button class="btn btn-primary btn-full" onclick="game.transitionTo(function() { game.renderHomeScreen() })">
                    חזרה לתפריט
                </button>
            </div>
        `;
