...
class SoundManager {
    constructor() {
        this.enabled = localStorage.getItem('boundariesGameSound') !== 'off';
        this.ctx = null;
    }
    ...
    toggle() {
        ...
        localStorage.setItem('boundariesGameSound', this.enabled ? 'on' : 'off');
        return this.enabled;
    }
}
...
class BoundariesGame {
    constructor() {
        ...
        // No story builder for this game

        // Mentor messages
        this.mentorMessages = {
            welcome: [
                "היי! אני דנה, ואני כאן ללוות אותך בדרך להורות מעצימה עם גבולות בריאים!",
                "ברוכים הבאים! אני דנה, המנטורית שלך לגבולות עם ילדים.",
                "שמחה לראות אותך! יחד נלמד לבנות אופי חזק אצל ילדינו."
            ],
            moduleIntro: {
                1: "הבסיס של הכל: למה ילדים צריכים גבולות ואיך זה בונה אופי. מוכנים?",
                2: "חוקי הגבולות הראשונים: זריעה וקצירה, אחריות וכוח. בואו נלמד!",
                3: "חוקי הכבוד, המוטיבציה והכאב — כלים חזקים להורות אפקטיבית!",
                4: "יוזמה, קנאה וחשיפה — העקרונות שמשלימים את התמונה!",
                5: "הגענו ליישום! ששת הצעדים להצבת גבולות בפועל. זה הרגע שהכל מתחבר!"
            },
            lessonStart: [
                "מעולה! בואו נתחיל. קחו את הזמן, אין לחץ.",
                "אני לצידך בכל שלב. בהצלחה!",
                "זוכרים — כל תשובה נכונה מקרבת אתכם להורות טובה יותר!"
            ],
            correctAnswer: [
                "מדהים! בדיוק ככה!",
                "וואו, את/ה ממש מבינ/ה את העקרונות!",
                "נכון מאוד! הילדים שלך יגדלו עם אופי חזק!",
                "יופי! גבול מדויק!",
                "מצוין! את/ה בדרך הנכונה!"
            ],
            wrongAnswer: [
                "לא נורא! טעויות הן חלק מהלמידה — גם בהורות.",
                "קרוב! חשבו על מה שבאמת בונה אחריות אצל הילד.",
                "לא בדיוק, אבל זו הזדמנות ללמוד משהו חדש!",
                "אל דאגה, גם ההורים הכי טובים לומדים כל יום!"
            ],
            encouragement: [
                "את/ה עושה עבודה נהדרת!",
                "כל תרגיל מקרב אותך להורות מעצימה!",
                "אני גאה בך!",
                "המשיכו ככה!"
            ],
            lessonComplete: [
                "איזה כיף! סיימת שיעור נוסף!",
                "מעולה! עוד צעד לקראת הורות עם גבולות בריאים!",
                "אלופים! הידע הזה ישנה את המשפחה שלכם!"
            ]
        };
        ...
        this.achievements = [
            ...
            { id: 'five_lessons', title: 'הורה לומד', desc: 'סיימת 5 שיעורים', icon: '📚', condition: (data) => Object.keys(data.completedLessons).length >= 5 },
            { id: 'all_lessons', title: 'מומחה גבולות', desc: 'סיימת את כל השיעורים', icon: '👑', condition: (data) => Object.keys(data.completedLessons).length >= 15 },
            { id: 'streak_3', title: 'התמדה', desc: '3 ימים ברצף', icon: '🔥', condition: (data) => data.streak >= 3 },
            { id: 'streak_7', title: 'שבוע של גבולות', desc: '7 ימים ברצף', icon: '⭐', condition: (data) => data.streak >= 7 },
            { id: 'streak_30', title: 'מחויבות הורית', desc: '30 ימים ברצף', icon: '💎', condition: (data) => data.streak >= 30 },
            { id: 'xp_500', title: 'צובר נקודות', desc: 'צברת 500 XP', icon: '🏅', condition: (data) => data.xp >= 500 },
            { id: 'xp_1000', title: 'אלוף הגבולות', desc: 'צברת 1000 XP', icon: '🏆', condition: (data) => data.xp >= 1000 },
            ...
            { id: 'module_complete', title: 'חוק שלם', desc: 'סיימת מודול שלם', icon: '🛡️', condition: (data) => Object.keys(data.completedLessons).length >= 3 },
            { id: 'ten_correct', title: 'עשר נכונות', desc: 'ענית נכון על 10 שאלות', icon: '✅', condition: (data) => data.totalCorrectAnswers >= 10 }
        ];

        // Reflection questions per module (for debriefing)
        this.reflectionQuestions = {
            1: [
                "חשבו על רגע אחרון שבו ילדכם ביקש משהו — האם הצבתם גבול? איך הרגשתם?",
                "איזה תכונת אופי תרצו לפתח אצל ילדכם? איך גבולות יכולים לעזור בזה?",
                "מה ההבדל בין הגנה על ילד לבין הצבת גבול? חשבו על דוגמה מהחיים."
            ],
            2: [
                "האם יש מצב שבו הגנתם על ילדכם מתוצאות טבעיות? מה קרה?",
                "חשבו על תחום שבו ילדכם לוקח אחריות — ותחום שבו לא. מה ההבדל?",
                "איפה בחיי ילדכם יש לו כוח בחירה אמיתי? איפה אתם יכולים להוסיף?"
            ],
            3: [
                "האם ילדכם מכבד את הגבולות שלכם? מה עוזר לו לכבד אותם?",
                "מה מניע את ילדכם — פרסים חיצוניים או סיפוק פנימי? איך אפשר לשנות?",
                "חשבו על כאב קצר-טווח שחסכתם מילדכם — האם זה עזר לו בטווח הארוך?"
            ],
            4: [
                "האם ילדכם נוטה להגיב או ליזום? איך אפשר לעודד יוזמה?",
                "האם קנאה השפיעה על ילדכם לאחרונה? איך טיפלתם בזה?",
                "באיזה תחום ילדכם צריך יותר חשיפה למציאות?"
            ],
            5: [
                "בחרו גבול אחד שתרצו ליישם השבוע — מה הוא ואיך תעשו את זה?",
                "חשבו על הצעד הכי קטן שתוכלו לעשות מחר בבוקר כדי ליישם גבול חדש.",
                "מי בסביבתכם יכול לתמוך בכם ביישום גבולות? שתפו אותם."
            ]
        };

        // Session tracking for adaptive hints
    ...
    async initAuth() {
        ...
        if (user) {
            // Returning user — skip landing, go straight to game
    ...
    getDefaultPlayerData() {
        return {
            ...
            perfectLessonsList: [],
            lessonBestScores: {},
            sessionAccuracy: { correct: 0, total: 0 }
        };
    }
        ...
        if (user) {
            try {
                ...
                if (data && !error) {
                    ...
                    localStorage.setItem('boundariesGameData', JSON.stringify(this.playerData));
                    return;
                }
    ...
    loadFromLocalStorage() {
        try {
            const saved = localStorage.getItem('boundariesGameData');
    ...
    async createSupabaseRow(user) {
        if (!user) return;
        try {
            await _supabase.from('boundaries_game_players').upsert({
                user_id: user.id,
    ...
    async saveToSupabase() {
        ...
        try {
            await _supabase.from('boundaries_game_players').update({
                xp: this.playerData.xp,
                level: this.playerData.level,
                hearts: this.playerData.hearts,
                max_hearts: this.playerData.maxHearts,
    ...
    renderOnboardingStep() {
        const steps = [
            {
                icon: '🛡️',
                title: 'ברוכים הבאים למשחק גבולות עם ילדים!',
                text: 'כאן תלמדו לבנות גבולות בריאים עם ילדיכם — צעד אחרי צעד, בדרך כיפית ואינטראקטיבית.'
            },
            {
                icon: '👩‍🏫',
                title: 'הכירו את דנה, המנטורית שלכם',
                text: 'דנה תלווה אתכם בכל שלב — תיתן טיפים, עידוד, ותעזור לכם להבין מה בונה אופי חזק אצל ילדים.'
            },
            {
                icon: '🚀',
                title: 'בואו נתחיל!',
        ...
        return `
            <div class="mentor-container mentor-enter">
                <div class="mentor-avatar mentor-${mood}">
                    <img src="mentor-dana.png" alt="דנה" />
                </div>
                <div class="mentor-bubble">
                    ${showName ? '<div class="mentor-name">דנה - המנטורית שלך</div>' : ''}
    ...
    getLevelName(level) {
        const names = ['הורה מתחיל', 'מציב גבולות', 'הורה אחראי', 'מומחה גבולות', 'הורה מעצים', 'מאסטר הורות', 'מנהיג משפחתי', 'אגדה'];
    ...
    renderHomeScreen() {
        ...
        MODULES.forEach((module, index) => {
            ...
            // Autonomy: First 3 modules always open, modules 4-5 require 50% on any previous module
            const isLocked = index >= 3 && !MODULES.slice(0, index).some(m => this.getModuleProgress(m.id) >= 50);
        ...
        pathNodesHtml += `
            <div class="home-path-node goal" style="cursor: default;">
                ...
                <div class="home-path-info">
                    ...
                    <div class="home-path-title">מומחה גבולות עם ילדים</div>
    ...
    startLesson(lessonId) {
        ...
        // Inject review exercises from queue (max 2)
        this.injectReviewExercises();

        this.showProgressBar();
        this.renderExercise();
    }

    injectReviewExercises() {
        if (!this.playerData.reviewQueue || this.playerData.reviewQueue.length === 0) return;

        const reviewCount = Math.min(2, this.playerData.reviewQueue.length);
        const reviewExercises = this.playerData.reviewQueue.splice(0, reviewCount);

        // Mark them as review exercises
        reviewExercises.forEach(ex => { ex._isReview = true; });

        // Insert review exercises at random positions in the lesson
        const exercises = [...this.currentLesson.exercises];
        reviewExercises.forEach(ex => {
            const pos = Math.min(exercises.length, 2 + Math.floor(Math.random() * (exercises.length - 1)));
            exercises.splice(pos, 0, ex);
        });

        this.currentLesson = { ...this.currentLesson, exercises };
        this.savePlayerData();
    }
    ...
    renderExercise() {
        ...
        // Add adaptive hint if struggling
        const hintHtml = this.getAdaptiveHint(exercise);
        if (hintHtml) {
            const exerciseContainer = container.querySelector('.exercise-container');
            if (exerciseContainer) {
                exerciseContainer.insertAdjacentHTML('afterbegin', hintHtml);
            }
        }

        // Add review badge if this is a review exercise
        if (exercise._isReview) {
            const exerciseContainer = container.querySelector('.exercise-container');
            if (exerciseContainer) {
                exerciseContainer.insertAdjacentHTML('afterbegin',
                    '<div class="review-badge">🔄 תרגיל חזרה</div>');
            }
        }
    ...
    createExerciseTip(exerciseType) {
        const tips = {
            'multiple-choice': "💡 חשבו מה באמת בונה אחריות ואופי אצל הילד.",
            'fill-blank': "💡 נסו לחשוב מה המילה שמשלימה את העיקרון בצורה הנכונה.",
            'order': "💡 חשבו על הסדר הנכון — מה הצעד הראשון ומה בא אחריו?",
            'identify': "💡 סמנו את הקטע בטקסט שמדגים את העיקרון הנכון.",
            'compare': "💡 דמיינו את עצמכם כהורים — איזו תגובה באמת תבנה גבולות בריאים?",
            'improve': "💡 חפשו את האפשרות שמציבה גבול ברור עם אהבה.",
            'match': "💡 חפשו את הקשר בין העמודות — מה מתחבר למה?"
        };
        ...
        container.innerHTML = `
            ...
            <div class="exercise-container">
                ...
                <div class="mentor-tip">
                    <img class="mentor-tip-icon" src="mentor-dana.png" alt="דנה" />
    ...
    renderImprove(container, exercise) {
        ...
        container.innerHTML = `
            <button class="back-btn" onclick="game.exitLesson()">✕</button>
            <div class="exercise-container">
                <div class="exercise-type">שפרו את הגישה</div>
                <div class="exercise-question">${exercise.question}</div>
    ...
    checkAnswer() {
        ...
        // Session accuracy tracking (for adaptive hints)
        this.sessionTotal++;
        ...
        if (isCorrect) {
            ...
            // Remove from review queue if it was a review exercise
            if (exercise._isReview) {
                // Successfully reviewed — no need to re-add
            }
        } else {
            ...
            // Add to review queue for spaced repetition (if not already a review)
            if (!exercise._isReview) {
                this.addToReviewQueue(exercise);
            }
        }
    ...
    completeLesson() {
        ...
        const completionTime = this.lessonStartTime ? Math.round((Date.now() - this.lessonStartTime) / 1000) : 0;

        // Track personal best
        if (!this.playerData.lessonBestScores) this.playerData.lessonBestScores = {};
        const prevBest = this.playerData.lessonBestScores[lessonKey];
        const isNewBest = !prevBest || this.lessonMistakes < prevBest.mistakes ||
            (this.lessonMistakes === prevBest.mistakes && completionTime < prevBest.time);

        if (isNewBest) {
            this.playerData.lessonBestScores[lessonKey] = {
                mistakes: this.lessonMistakes,
                time: completionTime,
    ...
    showCompletionScreen() {
        ...
        if (this._prevBest && this._isNewBest) {
            personalBestHtml = `
                <div class="personal-best-banner new-record">
                    <span class="personal-best-icon">🏅</span>
                    <span>שיא חדש! הפעם הקודמת: ${this._prevBest.mistakes} טעויות — עכשיו: ${this.lessonMistakes}!</span>
                </div>`;
        } else if (this._prevBest) {
            personalBestHtml = `
                <div class="personal-best-banner">
                    <span class="personal-best-icon">📊</span>
                    <span>השיא שלך: ${this._prevBest.mistakes} טעויות בזמן ${this.formatTime(this._prevBest.time)}</span>
                </div>`;
        }

        // Format completion time
        const timeStr = this.formatTime(this._completionTime);
        ...
        container.innerHTML = `
            <div class="completion-screen">
                ...
                <div class="completion-stats">
                    ...
                    <div class="completion-stat">
                        <div class="completion-stat-value">${this.lessonMistakes}</div>
                        <div class="completion-stat-label">טעויות</div>
                    </div>
                    <div class="completion-stat">
                        <div class="completion-stat-value">${timeStr}</div>
                        <div class="completion-stat-label">זמן</div>
                    </div>
                    <div class="completion-stat">
                ...
                <button class="btn btn-primary btn-full" onclick="game.transitionTo(function() { game.showReflectionScreen() })">
                    💭 רגע חשיבה
                </button>
                <button class="btn btn-secondary btn-full" style="margin-top: 10px;" onclick="game.transitionTo(function() { game.openModule(${this.currentModule.id}) })">
                    המשך ללמוד
                </button>
    ...
    // ═══════════════════════════════════════
    addToReviewQueue(exercise) {
        if (!this.playerData.reviewQueue) this.playerData.reviewQueue = [];
        // Don't add duplicates (check by question text)
        if (this.playerData.reviewQueue.some(e => e.question === exercise.question)) return;
        // Keep queue manageable (max 20)
        if (this.playerData.reviewQueue.length >= 20) this.playerData.reviewQueue.shift();
        // Clone the exercise without DOM references
        const clone = JSON.parse(JSON.stringify(exercise));
        this.playerData.reviewQueue.push(clone);
    }

    // ═══════════════════════════════════════
    // Reflection Screen (Debriefing)
    // ═══════════════════════════════════════
    showReflectionScreen() {
        const container = document.getElementById('game-container');
        const moduleId = this.currentModule.id;
        const questions = this.reflectionQuestions[moduleId] || this.reflectionQuestions[1];
        const question = questions[Math.floor(Math.random() * questions.length)];

        container.innerHTML = `
            <div class="completion-screen">
                <div class="completion-icon">💭</div>
                <div class="completion-title">רגע חשיבה</div>
                <div class="completion-subtitle" style="font-size: 1.1rem; line-height: 1.8; margin: 16px 0 24px;">
                    ${question}
                </div>
                ${this.createMentorHTML('קחו רגע לחשוב... הלמידה האמיתית קורה כשמחברים את התוכן לחיים.', false, 'idle')}
                <textarea class="reflection-textarea" id="reflection-answer" placeholder="כתבו את המחשבות שלכם כאן (לא חובה)..." rows="4"></textarea>
                <button class="btn btn-primary btn-full" style="margin-top: 16px;" onclick="game.closeReflection()">
                    המשך →
                </button>
            </div>
        `;
    ...
    // ═══════════════════════════════════════
    shouldShowHint() {
        if (this.sessionTotal < 3) return false;
        return (this.sessionCorrect / this.sessionTotal) < 0.5;
    }

    getAdaptiveHint(exercise) {
        if (!this.shouldShowHint()) return '';
        // Show a contextual hint based on exercise type
        const hints = {
            'multiple-choice': 'רמז: נסו לחשוב מה הילד באמת ילמד מכל אפשרות.',
            'fill-blank': 'רמז: קראו את המשפט בקול רם — איזו מילה נשמעת נכון?',
            'order': 'רמז: התחילו מהצעד הראשון הברור ביותר.',
            'compare': 'רמז: שימו לב — איזו תגובה מציבה גבול ברור עם אהבה?',
            'improve': 'רמז: חפשו את האפשרות שנותנת לילד אחריות.',
            'identify': 'רמז: חפשו את הביטוי שמדגים את העיקרון בצורה הכי ברורה.',
            'match': 'רמז: התחילו מהזוג שאתם הכי בטוחים בו.'
        };
        return `<div class="adaptive-hint">💡 ${hints[exercise.type] || ''}</div>`;
    }
